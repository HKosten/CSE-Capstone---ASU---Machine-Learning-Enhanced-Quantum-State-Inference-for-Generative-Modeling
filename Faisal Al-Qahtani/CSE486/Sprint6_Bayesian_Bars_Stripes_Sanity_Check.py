...
def is_bars_or_stripes(sample, rows=8, cols=16):
    grid = sample.reshape(rows, cols)

    # Horizontal stripes:
    # every row contains only one value
    horizontal = all(
        torch.all(row == row[0])
        for row in grid
    )
...
valid_count = sum(
    is_bars_or_stripes(sample)
    for sample in generated_samples
)

valid_percentage = (
    valid_count / len(generated_samples)
) * 100

print("Valid Bars/Stripes samples:", valid_count)
print("Validity percentage:", valid_percentage, "%")

real_valid_count = sum(
    is_bars_or_stripes(sample)
    for sample in test_data
)

real_valid_percentage = (
    real_valid_count / len(test_data)
) * 100

print("Real valid Bars/Stripes samples:", real_valid_count)
print("Real dataset validity:", real_valid_percentage, "%")